...
describe('fetchMavenVersion', () => {
    ...
    it('multi-versions', async () => {
        const versions = await network.fetchMavenMeta(
            testRepo,
            testGroupId,
            'malilib-1.12.1'
        )
    ...
    it('wrong-params', async () => {
        await expect(
            network.fetchMavenMeta(testRepo, testGroupId, 'unobtainium')
        ).rejects.toBeTruthy()
        await expect(
            network.fetchMavenMeta(testRepo, 'unobtainium', 'malilib-1.12.1')
        ).rejects.toBeTruthy()
        await expect(
            network.fetchMavenMeta(
                'http://example.com/',
                testGroupId,
                'malilib-1.12.1'
            )
