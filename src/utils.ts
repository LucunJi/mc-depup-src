...
export class GitHubVariables {
    readonly configPath = stringInput(
        'config_path',
        './.github/modding-dependencies.yml'
    )
...
export function typeOf<T>(x: T): TypeInfo {
    ...
    else if (typeof x === 'string' || x instanceof String) return 'string'
    else if (typeof x === 'number' || x instanceof Number) return 'number'
    else if (typeof x === 'boolean' || x instanceof Boolean) return 'boolean'
    else if (typeof x === 'symbol' || x instanceof Symbol) return 'symbol'
