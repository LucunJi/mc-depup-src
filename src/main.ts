...
export async function run(): Promise<void> {
    try {
        ...
        const depSettings = await DependencySettings.readFromFile(
            githubVars.configPath
        )

        const currMcVersionStr = properties.get(MINECRAFT_VERSION_KEY)
        if (currMcVersionStr === undefined)
            throw Error(
                `${MINECRAFT_VERSION_KEY} is not found in ${PROPERTIES_FILE}`
            )
        ...
        if (githubVars.updateMcPatch) {
            ...
            if (newerMcVersion) {
                core.info(
                    `${MINECRAFT_VERSION_KEY}: ${currMcVersionStr} => ${targetMcVersion.toString()}`
                )
                properties.set(
                ...
                core.info(
                    `${MINECRAFT_VERSION_KEY}: ${currMcVersionStr} => ${targetMcVersion.toString()} (no change)`
                )
            }
        } else {
        ...
        let updatedProps = 0
        const totalProps = depSettings.dependencies.flatMap((dep) =>
            Array.from(dep.properties.entries())
        ).length
        const summary: string[] = []
        if (needUpdateDep) {
            const tasks = depSettings.dependencies.map(
                async (dep: Dependency, i: number) =>
                    (async (): Promise<
                        ReturnType<typeof fetchDependencyUpdates>
                    > => {
                        try {
                            return await fetchDependencyUpdates(
                                dep,
                                targetMcVersion
                            )
                        } catch (error) {
                            ...
                            core.info(
                                `Skip error in fetching the latest version for the dependency at index ${i} due to tolerable=true`
                            )
            ...
            const newPropVals = results.flatMap((newValMap) =>
                Array.from(newValMap.entries())
            )
        ...
        if (!githubVars.dryRun) properties.save()
        else core.info('dry_run is set to true, the files will not be updated')
 ...
 */
async function fetchDependencyUpdates(
    ...
    for (const omitPatch of [false, true])
        for (
            let mcPatch = omitPatch ? 0 : targetMcVersion.patch;
            mcPatch >= 0;
            --mcPatch
        ) {
            if (mcPatch !== targetMcVersion.patch) await setTimeout(1000)

            const trialContext: DependencyContext = {
                mcVersion: new McVersion(
                    targetMcVersion.major,
                    targetMcVersion.minor,
                    mcPatch
                ),
            ...
            if (lastArtifactId !== artifactId) {
                try {
                    const meta = await fetchMavenMeta(
                    ...
                    core.debug(
                        `Trial failed for ${dependency.groupId}:${artifactId} in ${dependency.repository} with Minecraft version ${trialContext.mcVersion.toString(trialContext.omitMcPatch)}`
                    )
            ...
            for (const versionStr of versions) {
                ...
                if (
                    ...
                    parsedVersion.compare(bestVersion) >= 0
                ) {
            ...
            core.info(
                `The best matching version is ${bestVersionStr} for ${dependency.groupId}:${artifactId} in ${dependency.repository}`
            )
            ...
            for (const [
                propName,
                propAttrs
            ] of dependency.properties.entries()) {
                ...
                switch (propAttrs.source) {
                    ...
                    case 'artifactId':
                        ...
                        break
                    case 'wildcard': {
                        const expander = getContextualWildcardExpander(
                            propAttrs.wildcardName!
                        )
                        if (expander !== undefined) {
                            ...
                            for (
                                let j = 0;
                                j < dependency.version.length;
                                ++j
                            ) {
                                if (
                                    dependency.version[j].type ===
                                        'named_wildcard' &&
                                    dependency.version[j].value ===
                                        propAttrs.wildcardName
                                ) {
    ...
    throw new Error(
        `No matching version is found for ${dependency.groupId} in ${dependency.repository}`
    )
}
